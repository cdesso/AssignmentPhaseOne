...
module.exports = function(db, app) {
    app.post('/sendDeleteFromChannel', function(req, res){
        cName = req.body[0];
        gName = req.body[1];
        uName = req.body[2];

        var collection = db.collection('groups');
        collection.updateOne({'groupName': gName, 'channels.channelName': cName}, {'$pull':{'channels': {'members': {'username': uName}}}}, (err, docs)=>{
            if (err) throw err;
            res.send(true);
        })
    })
